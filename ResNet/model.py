...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, intermediate_channels, expansion, is_Bottleneck, stride):
        ...
        if self.in_channels == self.intermediate_channels * self.expansion: self.identify = True 
        else: 
            self.identify = False 
            self.projection = nn.Sequential(nn.Conv2d(in_channels=self.in_channels, out_channels=self.intermediate_channels*self.expansion, kernel_size=1, stride=stride, padding=0, bias=False ),
        ...
        if self.is_Bottleneck: 
            ...
            # 1x1 
            self.conv3_1x1 = nn.Conv2d(in_channels=self.intermediate_channels, out_channels=self.expansion, kernel_size=1, stride=1, padding=0, bias=False)
